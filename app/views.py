...
def edit_vpn_users():
    ...
    for ip in res_ip:
        s = s + str(ip) + '\n'
    form = EditVpnUserForm(vpn_login=user.name_vpn_users, email_vpn_users=user.email_vpn_users, adres_vpn=user.adres_vpn, allowedips_ip=s[:-1:], dz=user.organizations)
    if request.method == 'POST':
        result = request.form
